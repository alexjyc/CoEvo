...
class HybridRetriever(Module[RetrievalInput, RetrievalOutput]):
    ...
    def __init__(
        ...
        relevance_labels: Optional[Dict[int, List[int]]] = None,
    ...
    def set_relevance_labels(
        ...
        self.query_id_map = {
            self._hash_query(q['query']): q['query_id']
        ...
        def _sync_embed():
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    is_retryable = any(x in error_str for x in ['429', 'rate limit', 'connection', 'timeout', 'network'])
        ...
        def _safe_search():
            try:
                ...
                if query_vectors.ndim != 2 or query_vectors.shape[0] != len(queries):
                    print(f"Warning: Invalid embedding shape {query_vectors.shape}, expected ({len(queries)}, dim)")
                    return None, None
    ...
    async def run(self, input: RetrievalInput) -> RetrievalOutput:
        ...
        try:
            ...
            bm25_tasks = [
                asyncio.to_thread(self._retrieve_bm25, q, input.top_k)
                for q in queries
            ]
    ...
    async def find_optimal_weight(
        ...
        bm25_tasks = [
            asyncio.to_thread(self._retrieve_bm25, q, top_k)
        ...
        for w in weight_candidates:
            ...
            print(f"  {w:<8.1f} {s['precision']:<12.4f} {s['recall']:<12.4f} {s['f1']:<12.4f}{marker}")
    ...
    async def optimize_fusion_params(
        ...
        if query_id is None or query_id not in self.relevance_labels:
            return self._rrf_fusion_with_weight(
                bm25_results, dense_results, top_k, 0.5
            )
        ...
        async def test_weight(weight: float) -> Tuple[float, List[Dict[str, Any]], float]:
            fused = self._rrf_fusion_with_weight(
                bm25_results, dense_results, top_k, weight
            )
            retrieved_indices = {c["chunk_index"] for c in fused}
            hits = retrieved_indices & relevant_indices
