...
class DocumentPreprocessor:
    ...
    def generate_chunk_context(self, chunk: str, document: str) -> str:
        ...
        try:
            ...
            messages = [
                {
                    "role": "user",
                    "content": self.contextualizer_prompt.format(document=document, chunk=chunk),
                }
            ]

            response = self.client.chat.completions.create(
                model=self.contextualizer_model, messages=messages, temperature=0
            )
    ...
    def contextualize_chunks_parallel(self, chunks: list[str], document: str) -> list[str]:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in tqdm(
                as_completed(future_to_chunk), total=len(chunks), desc="Contextualizing chunks"
            ):
            ...
            for attempt in range(max_retries):
                try:
                    response = self.client.embeddings.create(
                        input=valid_texts, model=self.embedding_model
                    )
                    ...
                    if (is_rate_limit or is_connection) and attempt < max_retries - 1:
                        ...
                        print(
                            f"  Retry {attempt + 1}/{max_retries} after {delay:.1f}s: {str(e)[:50]}..."
                        )
                        time.sleep(delay)
                    else:
    ...
    async def generate_embeddings_async(self, texts: list[str]) -> list[list[float]]:
        ...
        print(
            f"Generating embeddings for {n_texts} texts (batch_size={batch_size}, concurrency={self.max_concurrent_batches})..."
        )
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if (is_rate_limit or is_connection) and attempt < max_retries - 1:
                            ...
                            print(
                                f"  {error_type} in batch {batch_idx}, retry {attempt + 1}/{max_retries}..."
                            )
    ...
    async def embed_texts_batch_async(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            idx_to_embedding = dict(zip(valid_indices, [data.embedding for data in response.data]))

            # Return embeddings in original order
            return [idx_to_embedding.get(i, [0.0] * embedding_dim) for i in range(len(texts))]
    ...
    def get_representative_data(
        ...
        for i in range(n_clusters):
            ...
            representatives.append(
                {
                    "query_id": evaluation_queries[rep_idx].get(
                        "query_id"
                    ),  # ✅ CRITICAL: Preserve query_id
                    "query": query_texts[rep_idx],
                    "ground_truth": evaluation_queries[rep_idx].get("ground_truth"),
                    "reference_doc_ids": evaluation_queries[rep_idx].get(
                        "reference_doc_ids"
                    ),  # ✅ Preserve references
                    "reference_evidence_texts": evaluation_queries[rep_idx].get(
    ...
    async def process_documents(
        self, documents: list[dict[str, Any]], use_context: bool = True
    ) -> None:
        """
        ...
        for doc_idx, doc in enumerate(documents):
            ...
            if "chunk_id" in doc:
                ...
                all_contextualized_chunks.append(
                    chunk_text
                )  # Will be contextualized below if needed
                ...
                for chunk_idx, (chunk, contextualized_chunk) in enumerate(
                    zip(chunks, contextualized_chunks)
                ):
        ...
        print(
            f"Generating embeddings for {len(embedding_input)} {'contextualized ' if use_context else ''}chunks..."
        )
    ...
    def load_relevance_labels(
        ...
        print("Converted document-level to chunk-level relevance:")
        ...
        if queries_with_no_chunks:
            print(
                f"  - {len(queries_with_no_chunks)} queries have no chunks (docs may not be indexed)"
            )
    ...
    def create_relevance_labels(
        ...
        format_str = (
            "chunk-level IDs"
            if using_chunk_ids
            else ("doc-level IDs (legacy)" if using_doc_ids else "no IDs found")
        )
    ...
    def load_index(self, index_path: str, metadata_path: str, use_context: bool = True) -> None:
        ...
        if hasattr(self, "chunk_metadata") and self.chunk_metadata:
            contextual_chunks = sum(
                1 for meta in self.chunk_metadata if meta.get("has_context", False)
            )
            total_chunks = len(self.chunk_metadata)
            print(
                f"Loaded {total_chunks} chunks with {len(self.chunk_id_to_index)} chunk IDs indexed"
            )
            if self.use_contextual_retrieval:
                print(
                    f"  Contextual Retrieval: {contextual_chunks}/{total_chunks} chunks have context"
                )
